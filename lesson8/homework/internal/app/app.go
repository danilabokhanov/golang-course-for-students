...
type App interface {
	...
	CreateUser(ctx context.Context, nickname, email string) (user.User, error)
...
type Users interface {
	...
	Add(ctx context.Context, nickname, email string) (int64, error)
...
func (d SimpleApp) CreateUser(ctx context.Context, nickname, email string) (user.User, error) {
	userID, err := d.users.Add(ctx, nickname, email)
	if err != nil {
		return user.User{}, ErrApp
	}
	u, _ := d.users.Find(ctx, userID)
	return u, nil
}
